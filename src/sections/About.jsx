            ...
            <div className="flex items-center gap-4">
              <div className="flex -space-x-3">
                {team.map((member, i) => (
                  <div
                    key={i}
                    className="w-10 h-10 rounded-full bg-accent/20 border-2 border-bg-card flex items-center justify-center text-xs font-bold text-accent"
                  >
                    {member.name
                      .split(" ")
                      .map((n) => n[0])
          ...
          <div className="grid grid-cols-2 gap-6">
            {stats.map((stat, index) => {
              ...
              return (
                <div
                  key={stat.label}
                  className={`bg-bg-card border border-border-subtle rounded-2xl p-8 text-center fade-in stagger-${index + 1}`}
                >
